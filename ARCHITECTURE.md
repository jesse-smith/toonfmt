...
- **Token storage:** `~/.toonfmt-auth/<key_hash>.json`, file `0600` / dir `0700`, one file per `(profile, url)` (holds rmcp's `StoredCredentials`). Simple, inspectable, cross-process — the `mcp-remote` model. The stem is `sha256(url)` with no `--profile` (byte-identical to the original per-URL scheme — existing logins keep working) or `sha256(profile ⊕ "\0" ⊕ url)` with one. **The file is created `0600` *before* the token bytes are written** (via `open(2)` mode in `write_private`, not a post-write chmod) — a Q3 hardening: the prior write-then-chmod left a sub-millisecond world-readable window on multi-user hosts. Do **not** revert to `tokio::fs::write` + `set_mode`. Dir perms (`0700`) are defense-in-depth (hide which upstreams exist); the file mode is the token's actual guard.
...
- **CI gate (`ci.yml`):** `cargo clippy --all-targets -- -D warnings` + a coverage run (`cargo llvm-cov --ignore-filename-regex 'src/main\.rs' --codecov`, which doubles as `cargo test`) uploaded to Codecov, on push/PR. **Coverage landed 2026-06-03** (plan `2026-06-03-coverage-quality`): the measured surface is the *coverable* one — `src/main.rs` (composition root: wiring + the platform browser spawn) is excluded via `--ignore-filename-regex` as inherently un-unit-testable; coverable-surface coverage is ~94%, the residual documented line-by-line in the plan. The Codecov upload uses `fail_ci_if_error: false` (the repo has no `CODECOV_TOKEN` yet — a missing-token error must not redden CI; set the secret to harden). **Complexity gate (same date):** `#![warn(clippy::cognitive_complexity)]` at the crate root + `cognitive-complexity-threshold = 20` in `clippy.toml`, made blocking by `-D warnings`. The one function over threshold — `http_upstream::run` @ 24, the protected handshake — carries an explicit `#[allow]` with a reason; re-check the nursery lint on toolchain bumps.
