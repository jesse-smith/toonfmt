...
**Commands.** The binary has three subcommands: the default **serve** form (an implicit `toonfmt [serve-flags]`, what `.mcp.json` invokes); **`toonfmt login --http <url>`** (a one-shot OAuth authorization, run by hand before serving — see OAuth below); and **`toonfmt update`** (self-update — see Language / distribution). `update` takes no arguments; on a non-installer build (no receipt) it prints guidance and exits 0 rather than erroring or clobbering.
