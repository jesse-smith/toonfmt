...
- **`install-updater = false`** — dist's standalone updater binary is declined in favor of a future in-binary `toonfmt update` subcommand (deferred Phase B, `axoupdater` as a library), keeping the single-binary ethos.
